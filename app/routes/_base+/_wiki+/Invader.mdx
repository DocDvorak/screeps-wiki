...
Strongholds are Invader bases that periodically spawn in a Source-keeper room in a sector. Only one can be spawned in a sector at a time. A level 1-5 Invader Core structure lies at the center of each stronghold. The higher the level of the invader core, the larger and more dangerous the stronghold will be.

A stronghold's towers will attack any player-owned creeps in the room. The energy in each tower will refill randomly once it dips below a certain threshold. Tower energy recharges occur more frequently in higher-level strongholds.

While they are quite a challenge to take down, strongholds contain many valuable resources See the [official docs](https://docs.screeps.com/invaders.html#Strongholds) for more details. Destroying a stronghold's core also disables invasions in the sector for a short time.
...
Shortly after a stronghold spawns, it will enter a deployed state (indicated by an effect on the invader core structure in the stronghold). While deployed, level 0 invader cores will periodically spawn in the unclaimed rooms with controllers within the sector. These cores will attack/reserve the room's controller until they expire or are destroyed. Since resources in a claimed or reserved room can only be harvested by the player that holds the claim/reservation, players should strive to keep their remotes clear of invader cores.
